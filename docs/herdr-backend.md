...
`bin/fm-bootstrap.sh`'s read-only session-start secondmate-liveness report needs the same underlying question the husk check above already answers with confidence: is this pane's agent actually alive, or is it a bare shell / gone pane pretending to be a live endpoint?
Rather than add a second herdr classifier, `fm_backend_herdr_agent_alive` (`bin/backends/herdr.sh`) is a thin wrapper around the already-verified `fm_backend_herdr_pane_agent_state`: `dead` and `no-agent` both collapse to the probe's `dead` verdict (a structurally-gone pane and a restored, agent-less bare shell are equally not a live secondmate - the exact shape a dead secondmate leaves behind), `live` maps to `alive`, and `unknown` stays `unknown`.
...
- **No automatic secondmate liveness recovery.** `fm_backend_agent_alive` provides the read-only session-start report in `bin/fm-bootstrap.sh`; a stopped coordinator resumes only through the captain's explicit project-local `secondmate <harness>` invocation.
  The watcher deliberately exempts secondmates from stale-pane detection because an idle secondmate pane is healthy by design.
