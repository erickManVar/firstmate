...
Limitations: zellij is experimental, has no per-home workspace split (all tasks share one tab bar, unlike herdr), has no verified agent-process liveness classifier for the read-only session-start secondmate report, and still carries the known gaps documented below (no native busy-state signal and a narrow focus-steal race on tab creation) - see "Known gaps left for a follow-up" at the end of this document.
...
| Agent liveness | *(no verified primitive)* | `fm_backend_agent_alive` reports `unknown` for zellij, so `bin/fm-bootstrap.sh`'s session-start secondmate liveness report marks the endpoint unproven and never changes it. |
| Endpoint state | `zellij list-sessions --short --no-formatting`, then `zellij action list-panes --json` filtered on the recorded pane id | `fm_backend_zellij_target_state` (`bin/backends/zellij.sh`) backs the read-only `fm_backend_target_state` dispatcher (`bin/fm-backend.sh`): an unlisted session or a listed session whose pane is absent is `missing`, a present pane whose tab matches the expected `fm-<id>` label is `exists`, and any CLI failure, unparseable output, or label mismatch is `unknown` - never treated as absence. |
...
- **No verified agent-process liveness classifier.** The session-start secondmate liveness report therefore receives `unknown` from `fm_backend_agent_alive` for zellij and reports `SECONDMATE_LIVENESS: ... liveness unproven` without changing the endpoint.
  The captain can inspect it and explicitly resume from the project container, avoiding a duplicate supervisor.
