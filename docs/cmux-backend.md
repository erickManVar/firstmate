...
| Endpoint state | `cmux workspace list --json --id-format uuids`, then `cmux list-panes --workspace <id> --json --id-format uuids` | `fm_backend_cmux_target_state` (`bin/backends/cmux.sh`) backs the read-only `fm_backend_target_state` dispatcher (`bin/fm-backend.sh`): a workspace or surface confirmed absent from the listings is `missing` (including a title-based re-resolve when the recorded uuid did not survive a relaunch), a listed surface matching the expected `fm-<id>` title is `exists`, and any CLI failure, unparseable output, or title mismatch is `unknown` - never treated as absence. |
