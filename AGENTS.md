...
1. **Never write to a project.**
   ...
   Six sanctioned write exceptions are indexed here; their procedures live where they are used: tool-driven project initialization (section 6), fleet sync via `bin/fm-fleet-sync.sh` (sections 3, 7, and 8), local-HEAD secondmate sync via explicit `bin/fm-spawn.sh --secondmate` launches (sections 3 and 7), inheritable config propagation via `bin/fm-config-push.sh` and explicit-launch convergence (sections 3 and 4), self-update via `/updatefirstmate` and `bin/fm-update.sh` (section 12), and approved `local-only` merge via `bin/fm-merge-local.sh` (section 7).
...
When it is set, scripts still use their own `bin/` from the repo they live in, but operational dirs come from `$FM_HOME`: `state/`, `data/`, and `config/`; project access defaults to `$FM_HOME/projects` and may use `config/projects-root` as documented in `docs/configuration.md`.
...
config/home-role  required explicit shared-container authorization: exactly `primary` permits canonical synchronization and exactly `secondmate` delegates it; LOCAL, gitignored, never inherited (docs/configuration.md)
...
5. **Fleet-state digest** - the full `data/backlog.md`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; and one cheap alive/dead/unknown read of each task's recorded backend endpoint, where `unknown` means the probe could not verify the endpoint either way, never that it is gone.
...
The locked fleet-sync sweep runs via `bin/fm-fleet-sync.sh` in the primary home only, best-effort and non-fatal, under the hard-rule exception in section 1; a marked secondmate home skips it and delegates canonical clone synchronization to the primary.
...
4. Use the `window=` values from the digest's `state/*.meta` entries as the live direct-report set, and read the digest's per-task `endpoint: alive|dead|unknown` line for each - that cheap check is already done; do not re-probe it yourself.
...
5. If the digest reports a recorded direct-report's endpoint as `dead` (or a meta has no `window=`), reconcile it through its meta as described below; an `unknown` endpoint is a probe failure, so inspect it before treating it as dead.
...
`data/secondmates.md` is the secondmate routing table: one line per persistent secondmate recording its id, charter summary, home path, natural-language scope, non-exclusive project access list, and added date.
The `scope:` field is used during intake; the `projects:` field is a non-exclusive access list, not ownership.
...
For `local-only`, create the local repo at the resolved `<projects-root>/<name>` path and skip GitHub entirely.
...
A project may appear in several `projects:` access lists, so choose the secondmate whose natural-language scope actually fits the work, such as triage versus feature development.
...
Before cancelling or changing a no-mistakes gate, load `gate-recovery` and obtain the captain's explicit approval.
...
When any wake's status reports a merged PR naming a project this home can access in its resolved catalog, run `bin/fm-fleet-sync.sh <project-name>` for that project as part of handling the wake; shared secondmate homes delegate this sync to the primary.
...
Keep the charter focused on persistent responsibility, available project access, escalation back to the main firstmate status file, and the idle-by-default contract: reconcile only its own in-flight work, then greet the captain and ask what they want to work on, never self-initiating a survey or audit.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints any diagnostic or capability line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `CREW_HARNESS_OVERRIDE:`, `CREW_DISPATCH:`, `FLEET_SYNC:`, `SECONDMATE_LIVENESS:`, `TASKS_AXI:`, or `FMX:`); silence needs no load.
...
- `gate-recovery` - load before inspecting, cancelling, or changing a no-mistakes gate that may need recovery.
