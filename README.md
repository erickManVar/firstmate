...
- **Optional secondmates** - opt in to persistent domain supervisors that run from isolated firstmate homes with their own `FM_HOME`, state, and session lock, using legacy local clones or explicit repos in a shared project container, including a real sibling `.secondmate` home; primary startup reports coordinator liveness only, while project-local `secondmate <harness>` explicitly starts or resumes and synchronizes a home when needed.
...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca when explicitly selected); kill the session anytime and the next one reconciles the primary fleet, while stopped secondmates remain reported until explicitly resumed from their project.
...
```
 ...
 ┌─────────────────────────────────────┐
 │ firstmate            (this repo)    │
 │ reads resolved projects + routes    │
...
- [docs/primary-entry.md](docs/primary-entry.md) - launch or resume the global primary firstmate session and receive its fleet recap.
- [docs/project-secondmate.md](docs/project-secondmate.md) - onboard, launch, inspect, and safely resume a project-level secondmate.
